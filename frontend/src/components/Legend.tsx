...
/**
 * Legend — collapsed by default (v0.6.0 UI fidelity): the graph owns the
 * screen; the full key appears on demand. Purely informational, read-only.
 */
export function Legend() {
  const [open, setOpen] = useState(false)
  return (
    <div className={`legend ${open ? 'open' : ''}`} onClick={() => setOpen(!open)}>
