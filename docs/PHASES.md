...
| **19** | Advanced Network Activity — per-connection byte/rate telemetry with honest capability tiers | **COMPLETE** (0.2.2: verified with REAL elevated Microsoft-Windows-TCPIP ETW — not synthetic telemetry. Modern Windows 11 manifest support: Tcb correlation (rundown/connect/accept → TCB cache → TcpDataTransferSend/Receive lookup → PID + real 4-tuple + direction + bytes), UDP self-contained events, readiness INITIALIZING/ACTIVE/DEGRADED; wildcard local-IP fallback (0.0.0.0/::) for outbound ETW attribution with ambiguity-safe single-candidate lookup + diagnostic counters; `tools/verify_tier2.ps1` 10/10 with real directional bytes; acceptance Tests R/S/T green with REAL DATA particles on real edges; 98 backend tests) |
